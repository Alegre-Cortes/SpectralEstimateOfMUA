...
%
%   Copyright 2018 Maurizio Mattia @ Ist. Super. Sanità, Rome - Italy
%   Version: 1.1 - Jun. 6, 2018
%
...
ndxF = find(F>MUAFreqBand(1)-2.0 & F<MUAFreqBand(2)+2.0);
% ndxF = find(F<=MUAFreqBand(2));
% ndxF = ndxF(2:end);

if exist('PyyBaseline','var') ~= 1
   PyyBaseline = mean(Pyy(ndxF,:), 2);
   PyyBaseline = PyyBaseline / PyyBaseline(ndxF(1));
end
if size(PyyBaseline,1) < size(PyyBaseline,2)
   PyyBaseline = PyyBaseline';
end

if exist('PlotPowerSpectrum','var') ~= 1
   PlotPowerSpectrum = 0;
end
